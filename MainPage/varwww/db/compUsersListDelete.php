...
$result = pg_prepare($db,"eId",'SELECT entity_id, contact_id FROM users WHERE username=$1');
$result = pg_prepare($db,"uList",'SELECT c.name, c.acc_name, c.title, c.phone, c.mobile, c.email FROM users as u, contacts as c WHERE u.entity_id=$1 AND c.id=u.contact_id AND c.id!=$2');
...
$result = pg_execute($db,"uList",array($eid,$cid));
