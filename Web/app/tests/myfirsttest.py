...
class MyFirstTest(unittest.TestCase):
    ...
    def testSingleBid(self):
        self.loan = self.createSampleLoan()
        self.user = self.createSampleUser()
        self.bid = self.createSampleBid(self.user, self.loan)
        bidsObjJson = self.bidToBidsJson(self.bid)
        bidsColletionUrl = "/bidWindow/bids"
        responseBody = self.getServerResponseBody(bidsColletionUrl)
        # json comparison
        self.assertEqual(responseBody, bidsObjJson) 

    @staticmethod                
    def createSampleLoan():
        # Creating a loan taken from model.loansModel.loansModelInstance
    ...
    def createSampleUser():
        ...
        return MyFirstTest.createDbUser(userEmail)
    
    @staticmethod
    def createDbUser(userEmail):
        apiUser = users.User(userEmail)
        # Ensures that we are not supposing that there's a logged user
        User.account.auto_current_user = False
        user = User(key_name = userEmail, account = apiUser)
        user.fundsAvailable = MyFirstTest.getRandomFundsAmount()
        user.put()
        return user
    
    @staticmethod
    def getRandomFundsAmount():
        # Amount in dollars
        return random.randint(100,1000) * 1000
    
    @staticmethod                
    def createSampleBid(user, loan):
        #Values for the bid
        participation = 10.0
        bidrate = 1.5
    ...
    @staticmethod                
    def bidToBidsJson(bid):
        bidsObj = MyFirstTest.bidToBidsObj(bid)
        return json.dumps(bidsObj)
    
    @staticmethod                
    def bidToBidsObj(bid):
        # Base json
        bidsObj = {}
        bidObj = {}
        bidId = bid.key().id_or_name()
        bidObj['bidId'] = bidId
        bidObj['date'] = bid.createdAt.strftime("%Y-%m-%d")
        bidObj['time'] = bid.createdAt.strftime("%H:%M:%S")
        bidObj['bidType'] = 'Specified'
        bidObj['participation'] = bid.participation
        bidObj['assetSubset'] = 'Loan'
        bidObj['loanId'] = bid.loan.key().id_or_name()
        bidObj['orderType'] = 'Competitive'
        bidObj['bidRate'] = bid.bidrate
        bidObj['orderTiming'] = 'Day Trade'
        bidObj['userId'] = bid.user.key().id_or_name()
        bidsObj[bidId] = bidObj
        return bidsObj
    
    def getServerResponseBody(self, url):
