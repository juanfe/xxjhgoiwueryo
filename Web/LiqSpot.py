...
class LiqEngine:
    ...
    def addLoans(self, idl, lo):
        ...
        try:
            ...
            try:
                ...
                sys.exit('Error: in the file %s, see the field delimiter in " \
                        "csv file, look "LiqSpot.py --help".' % self.options. \
                        loansFilename)
        try:
    ...
    def LoadMortgageOriginator(self):
        ...
        try:
            ...
            sys.exit('File %s, line %d: %s' % (self.options.OriginatorFilename,
                    fmo.line_num, e))
    ...
    def LoadLoans(self):
        ...
        if fileExt.lower() == ".json":
            ...
            #except:
            #    sys.exit('File %s, line %d: %s' % (self.options.loansFilename,
            #            flo.line_num, e))
        else:
            try:
    ...
    def SumRateAllocation(self, asset, assetSNC, ratesGC, WARateGNC):
        ...
        for k, bid in self.Bids.iteritems():
            ...
            rate = rate + (sum(map(lambda x,y: x*y, _LoanRates,
                assetSNC[k][0:-1]))/assetSNC[k][-1] if assetSNC[k][-1] != 0
                        else 0)
            rate = rate if asset[k][-1] > 0 else 0
            if ratesGC.has_key(k):
                rate = rate + ratesGC[k]['rateawarded']
            rate = rate if asset[k][-1] > 0 else 0
            if self.SpecifiedCompetitive(bid = k, specified = False,
                    competitive = False):
                rate += WARateGNC[-1]
            _all[k] = rate
        return _all
    ...
    def PrintSummary(self):
        ...
        if self.options.output:
            ofile = open(self.options.output, "wb")
            summwrt = csv.writer(ofile, delimiter=self.options.delimiter,
                    quotechar='"')
    ...
    def PrepareData(self, asset, AllocRates, WARateTot, GNComptAssetRem):
        ...
        for k, a in asset.iteritems():
            if k != 'Total' and a[-1] != 0:
                self.Data["bids"][k] = {"bidId": k,
                        "acceptedRate": 100 * AllocRates[k],
                        "allocatedAmounts": {},
                        "fundsTotal": a[-1]}
                for l in self.LoanIndex:
                    if a[self.LoanIndex.index(l)] != 0:
                        self.Data["bids"][k]["allocatedAmounts"][l] = a[self.
                                LoanIndex.index(l)]
    ...
    def CalcGC(self, assetSC, assetSNC, WARateS, SNCompAssetRem):
        ...
        if self.options.Verbose:
            print "General/Competitive bids are assigned to undersubscribed " \
                    "assets"
            print 50 * "-"
    ...
    def CalcGNC(self, assetSC, assetSNC, WARateS, WARateGC, MarketPremium,
        ...
        if self.options.Verbose:
            print "General/Noncompetitive bids are assigned to " \
                    "undersubscribed assets"
        allocateGNC = self.AllocateGenericNonCompetitive(GCompAssetRem)
        assetGNC = self.GenericAssetAssignation(Rem = GCompAssetRem,
                Allocate = allocateGNC)
    ...
    def Calc(self):
        ...
        assetSNC, WARateSNC, SNCompAssetRem = self.CalcSNC(assetSC,
                SCompAssetRem)
        ...
        WARateTot = self.WARateTot(assetSC, assetSNC, assetGC, assetGNC,
                WARateGNC, WARateSGC)
        # Make the summary of the assets
        asset = self.Summary(assetSC, assetSNC, assetGC, assetGNC, WARateGNC,
                WARateTot, GNComptAssetRem)
        AllocRates = self.SumRateAllocation(asset, assetSNC, ratesGC,
                WARateGNC)
